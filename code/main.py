...
output_dir='/data/pragya/VAE/output_dir'
checkpoint_dir='/data/pragya/VAE/checkpoint'
save_path='/data/pragya/VAE/generated/'
batch_path='/data/pragya/VAE/batch_path/'
# Task parameters
...
parser.add_argument('--output-dir', type=str, default='/data/pragya/VAE/output_dir', metavar='OD',
...
def execute_graph(epoch, model, fisher, data_loader, grapher, optimizer=None, prefix='test'):
    ...
    if 'train' not in prefix:
        ...
        epoch_path = os.path.join(batch_path, str(epoch))
        os.makedirs(epoch_path, mode=0o744, exist_ok=True)
        for i in range(0,len(images)):
              utils.save_image(images[i], os.path.join(epoch_path, str(img_names[i])+".png"))
...
def train_loop(data_loaders, model, fid_model, grapher, k, args):
    ...
    for j, loader in enumerate(data_loaders):
        num_epochs = 250 # TODO: randomize epochs by something like: + np.random.randint(0, 13)
        ...
        for epoch in range(1, num_epochs + 1):
            ...
            epochs=(k-1)*5+epoch
            train(epochs, model, fisher, optimizer, loader.train_loader, grapher)
            test_loss = test(epochs, model, fisher, loader.test_loader, grapher)
        ...
        # TODO: parameterize num fid samples, currently use less for inceptionv3 as it's COSTLY
                            ...
                            print("fid_score"+str(fid_score)+ "for_epoch"+str(num_epochs*k))
            #fid_loss_map = _add_loss_map(fid_loss_map, fid_score)
                            total_epochs=epoch
                            append_to_csv([fid_score],
                        os.path.join(args.output_dir, "fashion_fid.csv"))
                        os.makedirs(checkpoint_dir, mode=0o744, exist_ok=True)
        ...
        #checkpoint  = Variable(checkpoint).cuda() if args.cuda else Variable(checkpoint)
                        ...
                        path_check=os.path.join(checkpoint_dir, "checkpoint_{}.pth".format(k))
                        print("k_value",k)
                        k=k+1
                        fid_model.to(device)
                        print("saving checkpoint_"+str(k)+" to directory")
    ...
    for i in range(11, 12):
        if(i==0):
            ...
            if args.calculate_fid_with is not None:
                ...
                #optimizer = build_optimizer(model.student)
                #optimizer=None
                #VAE_train_loop(data_loaders, grapher, model, fid_model, i, optimizer, args)
                train_loop(data_loaders, model, fid_model, grapher, i, args)
     
        else:
            #if i<args.epochs-1:
            #_, data_loaders, grapher = get_model_and_loader()
            path_check=os.path.join(checkpoint_dir, "checkpoint_{}.pth".format(i-1))
            loaded_checkpoint=torch.load(path_check)
            e= loaded_checkpoint["epoch"]
            f= loaded_checkpoint["fid"]
            print("fid_score_after_epoch_: "+str((i-1)*iteration)+" is "+str(f))
            ...
            grapher1 = Grapher(env=model1.get_name(),
                          server=args.visdom_url,
                          port=args.visdom_port)
            print("training for iteration:"+str(i))
            #optimizer=None
            #VAE_train_loop(data_loaders, grapher1, model1, fid_model1, i,optimizer, args)
            train_loop(data_loaders, model1, fid_model1, grapher1, i, args)
