...
def calculate_fid(fid_model, model, loader, grapher,
    ...
    fid = np.inf
    fid = calculate_fid_from_generated_images(fid_model=fid_model, model=model, data_loader=loader,
                                              num_samples=num_samples, cuda=cuda)
    if grapher:
    ...
    with torch.no_grad():
        ...
        for data, _ in data_loader.test_loader:
            ...
            for begin, end in zip(range(0, batch_size, fid_model.batch_size),
                ...
                fid.append(frechet_gauss_gauss_np(generated_features.cpu().numpy(), test_features.cpu().numpy()))
...
def calculate_consistency(model, loader, reparam_type, vae_type, cuda=False):
    ...
    if model.current_model > 0 and reparam_type in ('mixture', 'discrete'):
        ...
        for img, _ in loader.test_loader:
            with torch.no_grad():
                ...
                teacher_posterior, student_posterior = zero_pad_smaller_cat(teacher_posterior, student_posterior)

                correct = to_data(teacher_posterior).max(1)[1] == to_data(student_posterior).max(1)[1]
                consistency.append(torch.mean(correct.type(float_type(cuda))).data.detach().cpu().numpy())
                samples_seen += img.size(0)

        num_test_samples = num_samples_in_loader(loader.test_loader)
        consistency      = np.mean(consistency)
        print("Consistency [#samples: {}]: {}\n".format(samples_seen, consistency))
