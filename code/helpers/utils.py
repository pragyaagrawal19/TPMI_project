...
def zero_pad_smaller_cat(cat1, cat2):
    ...
    if c1shp[1] > c2shp[1]:
        cat2 = torch.cat([cat2, Variable(float_type(cuda)(c2shp[0], diff).zero_())], dim=-1)
    elif c2shp[1] > c1shp[1]:
        cat1 = torch.cat([cat1, Variable(float_type(cuda)(c1shp[0], diff).zero_())], dim=-1)
    return [cat1, cat2]
...
def add_weight_norm(module):
    ...
    for param in params:
        if 'weight' in param or 'W_' in param or 'U_' in param:
            ...
            module = torch.nn.utils.weight_norm(module, param)
    return module
