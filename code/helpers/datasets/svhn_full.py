...
class SVHNFull(data.Dataset):
    ...
    split_list = {
        'train': ["http://ufldl.stanford.edu/housenumbers/train.tar.gz", "train.tar.gz", "a649f4cb15c35520e8a8c342d4c0005a"],
        'test' : ["http://ufldl.stanford.edu/housenumbers/test.tar.gz" , "test.tar.gz" , "790d9c8d42f1fcbd219b59956c853a81"],
        'extra': ["http://ufldl.stanford.edu/housenumbers/extra.tar.gz", "extra.tar.gz", "606f41243d71ca4d5fe66dbaf1f02bee"]
    }
    ...
    def __init__(self, root, split='train',
        ...
        imgs = [
            np.expand_dims(imresize(imread(os.path.join(basedir, '%d.png' % f)), size=[32, 32, 3], interp='lanczos'), 0)
    ...
    def create_dataset(self, data, path):
        ...
        for i in tqdm(range(len(data['/digitStruct/name']))):
            ...
            if num_length < 6:
                dd = {
                    'len': num_length,
                    'filename': os.path.join(path, '{}.png'.format(i+1))
                }
                for i in range(5):
                    dd[ 'num{}'.format(i+1)] = -1
                ...
                for i in range(num_length):
                    ...
                    dd['bbox{}'.format(i+1)] = (meta_data['left' ][i], meta_data['top'][i],
                                                meta_data['width'][i], meta_data['height'][i])
                dd['full_num'] = self.with_map(full_num)
            df.append(dd)

        df = pd.DataFrame(df)
        for i in range(1, 6):
            df.set_value(df[df['num{}'.format(i)] == 10].index, 'num{}'.format(i), 0)

        for i in range(1, 6):
            df.set_value(df['num{}'.format(i)].isnull(), 'num{}'.format(i), 10)

        for i in range(1, 6):
            for j in df['bbox{}'.format(i)][df['bbox{}'.format(i)].isnull()].index:
                df.set_value(j, 'bbox{}'.format(i+1), (0, 0, 0, 0))
